...
/**
 * Cheap shape-type check with no encoding involved - reused by {@link tryBuildCircularGeometryJSON}
 ...
 * @returns `"Circle"`/`"Arc"` if `shape` is one of the recognized circular shape types, else null.
 */
...
/**
 * Shared by {@link GMLFeatureEncoder} (schema-aware WFS-T encoding) and {@link AdvancedGMLCodec}
 ...
 * the shape's own properties sidesteps that entirely.
 *
 * @param shape   the shape to convert, if it's a Circle/Arc.
 * @param srsName the CRS identifier to stamp onto the result.
 * @param id      the id to stamp onto the result.
 * @throws {WFSTInvalidGeometry} if `shape` is a generic Arc with mismatched semi-major/semi-minor
 *         axes (`a !== b`) - GML 3.2 has no standard representation for a genuinely elliptical arc.
 * @returns the built {@link GMLCircle}/{@link GMLArc}, or null for every other shape type so
 ...
 * @returns `point` as a `[x, y, z]` tuple - always 3 elements (z defaults to 0 for a
 *          structurally-2D point, same as every other geometry type here); `encodeGeometryToGML`'s
 *          own `coordHasZ`/`mode3D` auto-detection decides 2D vs 3D output from whether z is
 *          actually non-zero, not from this array's length.
 */
